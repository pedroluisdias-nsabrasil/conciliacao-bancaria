...
def comprovantes():
    ...
    return [
        Comprovante(
            arquivo="comprovante1.pdf",
            data=date(2025, 11, 1),
            valor=Decimal("150.00")
        ),
        Comprovante(
            arquivo="comprovante2.pdf",
            data=date(2025, 11, 2),
            valor=Decimal("500.00")
        ),
    ]
...
def test_aba_resumo_tem_kpis(matches, lancamentos, estatisticas, tmp_path):
    ...
    assert any("66" in str(v) or "67" in str(v) for v in valores_celulas)  # Taxa conciliação
...
def test_aba_conciliados_tem_headers(matches, lancamentos, estatisticas, tmp_path):
    ...
    headers_esperados = ["Data", "Tipo", "Valor", "Descrição", "Comprovante", "Confiança", "Status"]
    
    for col, header_esperado in enumerate(headers_esperados, start=1):
        header_real = ws.cell(1, col).value
        assert header_real == header_esperado, f"Coluna {col}: esperado '{header_esperado}', obtido '{header_real}'"
