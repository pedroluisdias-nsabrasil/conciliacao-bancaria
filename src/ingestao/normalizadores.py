...
def normalizar_valor(texto: str) -> Decimal:
    ...
    try:
        ...
        raise ValueError(f"Não foi possível converter '{texto}' para valor monetário: {e}")
...
def identificar_tipo_lancamento(descricao: str, valor: Decimal, coluna_tipo: Optional[str] = None) -> str:
    """
    ...
    palavras_debito = [
        'PAGAMENTO', 'PAG', 'TED', 'DOC', 'PIX ENVIADO', 'TRANSFERENCIA',
        'COMPRA', 'SAQUE', 'TARIFA', 'IOF', 'JUROS', 'MULTA',
        'DEBITO', 'DÉBITO', 'ENVIO', 'ENVIADO'
    ]
    
    # Palavras que indicam crédito (entrada de dinheiro)
    palavras_credito = [
        'DEPOSITO', 'DEPÓSITO', 'PIX RECEBIDO', 'RECEBIDO', 'CREDITO',
        'CRÉDITO', 'ESTORNO', 'DEVOLUCAO', 'DEVOLUÇÃO', 'RESGATE',
        'RENDIMENTO', 'APLICACAO', 'APLICAÇÃO'
    ]
