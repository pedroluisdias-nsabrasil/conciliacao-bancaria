...
class LeitorCSV:
    ...
    def ler_arquivo(
        ...
        # Verificar extensão ANTES de verificar existência
        ...
        try:
            ...
            logger.info(
                f"Arquivo lido: {len(self._df)} linhas, {len(self._df.columns)} colunas"
            )
    ...
    def _detectar_formato(self) -> str:
        ...
        logger.warning(
            "Formato não reconhecido especificamente, usando mapeamento genérico"
        )
        return "generico"
    ...
    def _mapear_colunas(self) -> None:
        ...
        mapeamento_banco = self.MAPEAMENTOS.get(
            self.banco, self.MAPEAMENTOS["generico"]
        )
