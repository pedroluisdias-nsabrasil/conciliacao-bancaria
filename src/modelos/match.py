...
class Match:
    ...
    def __post_init__(self):
        ...
        if not 0.0 <= self.confianca <= 1.0:
            raise ValueError(
                f"Confiança deve estar entre 0.0 e 1.0, " f"recebido: {self.confianca}"
            )
        ...
        if self.comprovante is not None and not isinstance(
    ...
    def __str__(self) -> str:
        ...
        """
        comp_info = (
            self.comprovante.nome_arquivo
            if self.comprovante
            else "Sem comprovante (regra)"
        )
